 ...
 */
public class GMServerSets {

  private GMServerSets() {
    // Utility class.
  }
   ...
   */
  public static GMServerSet create(ZooKeeperClient zkClient, Iterable<ACL> acl, String zkPath) {
   ...
   */
  public static GMServerSet create(ZooKeeperClient zkClient, Iterable<ACL> acl, Set<String> zkPaths) {
    Preconditions.checkNotNull(zkClient);
    MorePreconditions.checkNotBlank(acl);
    MorePreconditions.checkNotBlank(zkPaths);

    if (zkPaths.size() == 1) {
      return new GMServerSetImpl(zkClient, acl, Iterables.getOnlyElement(zkPaths));
    } else {
      ImmutableList.Builder<GMServerSet> builder = ImmutableList.builder();
      for (String path : zkPaths) {
        builder.add(new GMServerSetImpl(zkClient, acl, path));
      }
      return new GMCompoundServerSet(builder.build());
    }
  }
  ...
  public static Endpoint toEndpoint(InetSocketAddress address) {
    return new Endpoint(address.getAddress().getHostAddress(), address.getPort());
  }
}