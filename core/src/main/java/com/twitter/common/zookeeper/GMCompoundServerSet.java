 ...
 */
public class GMCompoundServerSet implements GMServerSet {
  private static final Joiner STACK_TRACE_JOINER = Joiner.on('\n');

  private final List<GMServerSet> serverSets;
  private final Map<GMServerSet, ImmutableSet<ServiceInstance>> instanceCache = Maps.newHashMap();
   ...
   */
  public GMCompoundServerSet(Iterable<GMServerSet> serverSets) {
    MorePreconditions.checkNotBlank(serverSets);
    this.serverSets = ImmutableList.copyOf(serverSets);
  }

  private interface JoinOp {
    EndpointStatus doJoin(GMServerSet serverSet) throws JoinException, InterruptedException;
  }
  ...
  private EndpointStatus doJoin(JoinOp joiner) throws JoinException, InterruptedException {
    ...
    for (GMServerSet serverSet : serverSets) {
  ...
  public EndpointStatus join(
    ...
    return doJoin(new JoinOp() {
      @Override public EndpointStatus doJoin(GMServerSet serverSet)
  ...
  private synchronized void handleChange(GMServerSet serverSet, ImmutableSet<ServiceInstance> hosts) {
  ...
  public synchronized Command watch(HostChangeMonitor<ServiceInstance> monitor)
    ...
    if (stopWatching == null) {
      ...
      for (final GMServerSet serverSet : serverSets) {
