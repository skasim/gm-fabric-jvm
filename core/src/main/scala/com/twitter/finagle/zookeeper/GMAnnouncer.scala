...
import com.twitter.common.zookeeper.GMServerSet.EndpointStatus
import com.twitter.common.zookeeper.{GMServerSet, GMServerSetImpl, ZooKeeperClient}
...
/**
  ...
  */
class GMAnnouncerException(msg: String) extends Exception(msg)
...
/**
  ...
  */
class GMAnnouncer(factory: ZkClientFactory) extends Announcer { self =>
  val scheme = "gm"
  ...
  private[this] val mutator = new Thread("GmAnnouncer Mutator") {
    setDaemon(true)
    start()
  ...
  def announce(
    ...
    val conf = serverSets find { s => s.client == client && s.path == path && s.shardId == shardId } getOrElse {
      val serverSetConf = ServerSetConf(client, path, shardId, new GMServerSetImpl(client, path))
